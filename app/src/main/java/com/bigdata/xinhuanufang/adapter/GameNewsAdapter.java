...
public class GameNewsAdapter extends BaseAdapter {
	...
	public GameNewsAdapter(Context context, List<NewsBean> list) {
	...
	public View getView(int	position, View convertView, ViewGroup parent) {
		...
		//时间格式转换
		String strTime = configUtils.getStrTimes((list.get(0).getmList2().get(position)).getNews_date());
		Picasso.with(context).load(Config.ip + list.get(0).getmList2().get(position).getNews_pic()).into(holder.tv_picture);
//		x.image().bind(holder.tv_picture, Config.ip + list.get(0).getmList2().get(position).getNews_pic());
		holder.tv_title.setText((list.get(0).getmList2().get(position)).getNews_title());
		holder.tv_date.setText(strTime);
		holder.tv_title2.setText((list.get(0).getmList2().get(position)).getNews_title2());
