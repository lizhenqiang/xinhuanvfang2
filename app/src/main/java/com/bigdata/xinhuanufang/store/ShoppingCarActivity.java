...
import com.bigdata.xinhuanufang.adapter.ShoppingCarAdapter.onCheckBoxChangeListener;
import com.bigdata.xinhuanufang.adapter.ShoppingCarAdapter.onClick;
...
import static com.bigdata.xinhuanufang.R.id.tv_asc_money;

/**
 * @author asus 购物车Activity
 */
public class ShoppingCarActivity extends BaseActivity implements OnClickListener {
	...
	protected void showListView(final List<NetworkDataCar> shopingCarData) {
		// 查找到list控件
        shoppingCarLV = (ListView) findViewById(R.id.lv_shoppingC_shopingCar);
		all_Choose=(CheckBox) findViewById(R.id.cb_asc_choiceOrNot);
		shopingcar_number=(TextView) findViewById(R.id.shopingcar_number);
		all_Choose.setOnClickListener(this);
		shoping_money=(TextView) findViewById(tv_asc_money);
		//结算按钮
		btn_asc_emptyAll = (Button) findViewById(R.id.btn_asc_emptyAll);
		btn_asc_emptyAll.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				...
				if (!isquanxuan) {
					Toast.makeText(ShoppingCarActivity.this, "请选择支付商品", Toast.LENGTH_SHORT).show();
					return;
				}
				//调起支付
				//拿到商品信息
...
//				int position=positionIndex;
				...
				System.out.println("购物车"+ls.size());
				for (int i = 0; i < ls.size(); i++) {
					System.out.println("购物车--------"+i);
		...
		shoppingCarAdapter.setOnClick(new onClick() {
		...
		ls = new HashMap<>();
		shoppingCarAdapter.setOnBoxChangeListener(new onCheckBoxChangeListener() {
			...
			public void onChanged(int position, boolean isChecked,String isquanchoose,int size) {
				...
				if (isChecked) {
					...
					//显示金额
					System.out.println("444"+isquanchoose);
					...
					public void onSuccess(String arg0) {
						...
						if (TextUtils.isEmpty(arg0)) {
							...
							try {
								...
								List<cart> mCart=new ArrayList<cart>();
								for (int i = 0; i < cart.length(); i++) {
									...
									//存储attrs发热数据
									List<attrs> mAttrs=new ArrayList<attrs>();
