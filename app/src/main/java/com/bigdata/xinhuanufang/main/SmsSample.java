...
public class SmsSample {
    ...
    public SmsSample(String phoneNumber) {
        ...
        String testUsername = "lizhenqiang"; //在短信宝注册的用户名
        String testPassword = "1005asdfghjkll"; //在短信宝注册的密码
        random = (int) ((Math.random()*9+1)*100000);
        String testPhone = phoneNumber;
        String testContent = "【心花怒放】您的验证码是"+ random+",５分钟内有效。若非本人操作请忽略此消息。"; // 注意测试时，也请带上公司简称或网站签名，发送正规内容短信。千万不要发送无意义的内容：例如 测一下、您好。否则可能会收不到
