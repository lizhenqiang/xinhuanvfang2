...
public class PullToRefreshRecyclerView extends RecyclerView {
    ...
    public RefreshHead refreshHeader;
    ...
    public boolean onTouchEvent(MotionEvent e) {
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                lastY = e.getRawY();
                Log.e("DOWN", "lastY=="+lastY);
                break;
            case MotionEvent.ACTION_MOVE:
                float moveY = e.getRawY() - lastY;

                Log.e("MOVE", "moveY=="+moveY+"getRawY=="+e.getRawY()+"lastY=="+lastY);
                //这个地方老子也不知道为啥lastY会等于-1但实际测试出现了down事件没有执行，lastY=-1。
                if(lastY==-1) {
                    moveY=0;
                }
            ...
            case MotionEvent.ACTION_UP:
                //实际测试中down事件有肯能被屏蔽,这个-1是实际down被屏蔽时测出来的值。
                lastY = -1;
